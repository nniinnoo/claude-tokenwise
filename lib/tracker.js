...
/**
 * Parses Claude's /context markdown response to find exactly the tokens used.
 * E.g., extracts "6.2k / 200k (3%)" from "**Tokens:** 6.2k / 200k (3%)"
 */
export function parseContextTokens(text) {
  if (!text) return null;
  const match = text.match(/\*\*Tokens:\*\*\s*(.+?)\n/);
